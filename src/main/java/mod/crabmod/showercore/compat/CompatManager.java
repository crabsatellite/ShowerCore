 ...
 * - Comprehensive error logging with diagnosis
...
public class CompatManager {
    ...
    public static void registerCompat(String modId, String displayName,
        ...
        REGISTERED_COMPATS.put(modId,
                new CompatInfo(modId, displayName, isLoaded, initializer, requiredApiClasses));
    }
        ...
        for (Class<?> handlerClass : handlerClasses) {
            try {
                ...
                throw new RuntimeException(
                        "Event handler registration failed: " + handlerClass.getSimpleName(), e);
            }
        }
    }
     ...
     * This should be called early to fail fast with a clear message rather than
     * crashing later at runtime with a confusing NoClassDefFoundError.
     *
     ...
     */
    public static <T> T safeEventCall(String modId, String eventName,
    ...
    private static void handleCompatError(CompatInfo compat, Throwable error) {
        ...
        if (error instanceof NoClassDefFoundError || error instanceof ClassNotFoundException) {
            ...
            LOGGER.error("DIAGNOSIS: {} added new abstract methods that ShowerCore hasn't implemented.",
                    compat.displayName);
    ...
    private static void handleRuntimeError(String modId, String operationName, Throwable error) {
        ...
        if (error instanceof NoClassDefFoundError || error instanceof ClassNotFoundException) {
            ...
            LOGGER.error("DIAGNOSIS: An API method/field in {} was removed or its signature changed.",
                    compat.displayName);
            LOGGER.error("  Changed API element: {}", error.getMessage());
        } else if (error instanceof AbstractMethodError) {
            LOGGER.error("DIAGNOSIS: {} requires new methods that ShowerCore hasn't implemented yet.",
                    compat.displayName);
            LOGGER.error("  Method: {}", error.getMessage());
