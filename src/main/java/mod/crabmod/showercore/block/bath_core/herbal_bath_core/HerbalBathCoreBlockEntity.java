...
public class HerbalBathCoreBlockEntity extends BlockEntity {
  ...
  public HerbalBathCoreBlockEntity(BlockPos pPos, BlockState pBlockState) {
    super(BlockEntitiesRegister.HERBAL_BATH_CORE_BLOCK_ENTITY.get(), pPos, pBlockState);
  }
  ...
  public static void clientTick(
      Level pLevel, BlockPos pPos, BlockState pState, HerbalBathCoreBlockEntity pBlockEntity) {
  ...
  public static void serverTick(
    ...
    if (i % 40L == 0L) {
      ...
      if (flag != pBlockEntity.isActive) {
        SoundEvent soundevent =
            flag ? SoundEvents.CONDUIT_ACTIVATE : SoundEvents.CONDUIT_DEACTIVATE;
    ...
    if (pBlockEntity.isActive()) {
      if (i % 80L == 0L) {
        pLevel.playSound(
            (Player) null, pPos, SoundEvents.CONDUIT_AMBIENT, SoundSource.BLOCKS, 0.5F, 1.0F);
      }
      ...
      if (i > pBlockEntity.nextAmbientSoundActivation) {
        ...
        pLevel.playSound(
            (Player) null, pPos, SoundEvents.CONDUIT_AMBIENT_SHORT, SoundSource.BLOCKS, 0.5F, 1.0F);
      }
    }
  }

  private static void updateHunting(
          HerbalBathCoreBlockEntity pBlockEntity, List<BlockPos> pPositions) {
      ...
      for (int k1 = -2; k1 <= 2; ++k1) {
        for (int l1 = -2; l1 <= 2; ++l1) {
          ...
          if ((i2 > 1 || l > 1 || i1 > 1)
            ...
            if (isBathCoreFrame(blockstate, pLevel, blockpos1, pPos)) {
  ...
  private static boolean isBathCoreFrame(
    ...
    for (Block block : Config.herbalBathCoreBlocks) {
  ...
  private static void applyEffects(Level pLevel, BlockPos pPos, List<BlockPos> pPositions) {
    ...
    AABB aabb =
        (new AABB(
                ...
                (double) (i1 + 1)))
            .inflate((double) j)
            .expandTowards(0.0D, (double) pLevel.getHeight(), 0.0D);
    List<Player> list = pLevel.getEntitiesOfClass(Player.class, aabb);
    if (!list.isEmpty()) {
      for (Player player : list) {
        if (pPos.closerThan(player.blockPosition(), (double) j)) {
          player.addEffect(new MobEffectInstance(MobEffects.REGENERATION, 260, 1, false, false));
        }
      }
    }
  }

  private static void updateDestroyTarget(
      Level pLevel,
      BlockPos pPos,
      BlockState pState,
      List<BlockPos> pPositions,
      HerbalBathCoreBlockEntity pBlockEntity) {
    ...
    } else if (!pBlockEntity.destroyTarget.isAlive()
        || !pPos.closerThan(pBlockEntity.destroyTarget.blockPosition(), 8.0D)) {
      pBlockEntity.destroyTarget = null;
    }
  ...
  private static void updateClientTarget(
      Level pLevel, BlockPos pPos, HerbalBathCoreBlockEntity pBlockEntity) {
    if (pBlockEntity.destroyTargetUUID == null) {
      pBlockEntity.destroyTarget = null;
    } else if (pBlockEntity.destroyTarget == null
        || !pBlockEntity.destroyTarget.getUUID().equals(pBlockEntity.destroyTargetUUID)) {
      pBlockEntity.destroyTarget = findDestroyTarget(pLevel, pPos, pBlockEntity.destroyTargetUUID);
      if (pBlockEntity.destroyTarget == null) {
        pBlockEntity.destroyTargetUUID = null;
      }
