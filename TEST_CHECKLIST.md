...
- [ ] **选取方块 (Pick Block)**：
  - [ ] 在创造模式下，对着一个**已安装核心**的喷头按中键（选取方块）。
  - [ ] **预期结果**：
    - 获得的喷头物品包含 NBT 数据。
    - 物品提示显示“内含：[核心名称]”。
    - 放置该物品后，新喷头内自动包含相同的核心。
...
- [ ] **基本请求流程**：
  ...
  - [ ] 玩家 B 右键点击同一个浴缸。
  - [ ] **预期结果**：
    - 玩家 A 收到聊天栏提示，包含 [Accept] 和 [Reject] 按钮。
    - 玩家 B 收到提示 "Asking [Player A] for permission..."。
    - 玩家 B **没有**立即坐入浴缸。
- [ ] **接受请求**：
  - [ ] 玩家 A 点击 [Accept] 按钮。
  - [ ] **预期结果**：
...
- [ ] **距离限制**：
  ...
  - [ ] 玩家 A 点击 [Accept]。
  - [ ] **预期结果**：
    - 玩家 A 收到 "You are too far apart!" 错误提示。
    - 玩家 B 未进入浴缸。
- [ ] **主位空缺时的加入逻辑（三人场景）**：
  - [ ] 玩家 A（主位/Head）和玩家 B（副位/Foot）正在共浴。
  - [ ] 玩家 A 离开浴缸，玩家 B 留在副位。
  - [ ] 玩家 C 右键点击浴缸尝试加入。
  - [ ] **预期结果**：
    - 玩家 B **保持在副位不动**（不被强制移动到主位）。
    - 玩家 B 收到玩家 C 的加入请求。
    - 玩家 B 点击 [Accept] 后，玩家 C 自动坐入**主位（Head）**。
...
## 7. 配置文件测试 (Configuration)

### 7.1 核心激活方块 (Core Activation Blocks)

- [ ] **默认配置验证**：
  - [ ] **热水核心**：在喷头周围放置海晶石或海晶石砖，验证核心是否激活。
  - [ ] **蜂蜜浴核心**：在喷头周围放置蜜脾块或蜂蜜块，验证核心是否激活。
  - [ ] **牛奶浴核心**：在喷头周围放置石英块系列，验证核心是否激活。
  - [ ] **玫瑰浴核心**：在喷头周围放置红色羊毛/混凝土/带釉陶瓦，验证核心是否激活。
  - [ ] **牡丹浴核心**：在喷头周围放置粉色羊毛/混凝土/带釉陶瓦，验证核心是否激活。
  - [ ] **草药浴核心**：在喷头周围放置苔藓、藤蔓或树叶，验证核心是否激活。
- [ ] **结构缺失测试**：
  - [ ] 移除上述核心周围的指定方块。
  - [ ] **预期结果**：核心停止工作，不再给予 Buff，或有视觉提示表明结构不完整。
- [ ] **自定义方块配置**：
  - [ ] 修改 `showercore-common.toml`，将某核心（如热水核心）的激活方块改为其他方块（如泥土 `minecraft:dirt`）。
  - [ ] 重启游戏或重载配置。
  - [ ] **预期结果**：
    - 原有的海晶石不再能激活核心。
    - 放置泥土后核心成功激活。

### 7.2 客户端视觉配置 (Client Visuals)

- [ ] **半透明粒子开关**：
  - [ ] 找到 `showercore-client.toml` 配置文件。
  - [ ] **开启半透明**：设置 `enable_translucent_particles = true`。
    - **预期结果**：游戏内喷头产生的粒子（如玫瑰浴、牡丹浴）呈现半透明效果，类似水雾。
  - [ ] **关闭半透明**：设置 `enable_translucent_particles = false`。
    - **预期结果**：游戏内喷头产生的粒子呈现不透明效果，颜色更加实心鲜艳。

### 7.3 蒸汽产生流体 (Steam Fluids)

- [ ] **默认行为**：
  - [ ] 在浴缸中倒入岩浆。
  - [ ] **预期结果**：默认情况下不产生蒸汽粒子（除非岩浆被硬编码为产热流体，否则应遵循配置）。
- [ ] **添加流体**：
  - [ ] 在 `showercore-common.toml` 的 `steam_fluids` 列表中添加 `minecraft:lava`。
  - [ ] 重启游戏。
  - [ ] **预期结果**：装有岩浆的浴缸上方现在会产生蒸汽粒子效果。

### 7.4 小黄鸭销毁流体 (Rubber Duck Destruction)

- [ ] **默认销毁**：
  - [ ] 将小黄鸭放入岩浆。
  - [ ] **预期结果**：小黄鸭被销毁。
- [ ] **自定义销毁流体**：
  - [ ] 在 `showercore-common.toml` 的 `rubber_duck_destroy_fluids` 列表中添加 `minecraft:water`（仅用于测试）。
  - [ ] 重启游戏。
  - [ ] **预期结果**：将小黄鸭放入水中时，它也会被销毁（模拟强酸或其他危险液体）。
